...
const DetailPage: React.FC = () => {
  ...
  const umkm = (umkmData as Umkm[]).find(
  ...
  return (
    <AnimatedWrapper>
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100/50 pt-20">
        {/* Header */}
                ...
                {umkm.placeGallery && umkm.placeGallery.length > 0 ? (
                  ...
                  <div className="bg-slate-50 rounded-xl p-12 text-center border border-slate-100/50">
                    ...
                    <p className="text-slate-600 font-medium">
                      Tidak ada foto tempat tersedia
                    </p>
            ...
            <div className="lg:col-span-1 lg:sticky lg:top-24 lg:self-start space-y-6">
              ...
              >
                ...
                <motion.button
                  ...
                  whileTap={{
                    scale: 0.98,
                    boxShadow: "0 2px 10px -3px rgba(16, 185, 129, 0.2)",
                  }}
                  transition={{
                    type: "spring",
                    stiffness: 300,
                  ...
                  className="w-full flex items-center justify-center space-x-2 bg-gradient-to-r from-primary-500 to-primary-600 hover:from-primary-600 hover:to-primary-700 text-white px-6 py-3.5 rounded-xl font-medium shadow-md transition-all duration-300 relative overflow-hidden group"
                >
                  <motion.div
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {(umkmData as Umkm[])
                .filter(
                  (item) =>
                    item.category === umkm.category && item.id !== umkm.id
                )
                .slice(0, 3)
                .map((relatedUmkm) => (
                  <UmkmCard key={relatedUmkm.id} umkm={relatedUmkm} />
                ))}
            </div>
            {(umkmData as Umkm[]).filter(
              (item) => item.category === umkm.category && item.id !== umkm.id
            ).length === 0 && (
              <div className="text-center py-12 bg-slate-50 rounded-2xl border border-slate-100">
                <p className="text-slate-500 text-lg">
                  Tidak ada UMKM serupa yang ditemukan
                </p>
              </div>
            )}
          </motion.div>
